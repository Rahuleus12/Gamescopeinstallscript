...
# Terminal GUI for GSI.sh script
# This script provides a user-friendly interface for the GSI.sh script
...
# Check if GSI.sh exists
if [ ! -f "./GSI.sh" ]; then
    dialog --title "Error" --msgbox "GSI.sh script not found in the current directory.\nPlease make sure you're running this script from the same directory as GSI.sh." 8 60
    exit 1
fi

# Make sure GSI.sh is executable
chmod +x ./GSI.sh

# Function to display the main menu
show_main_menu() {
    dialog --clear --title "Gamescope Installation Manager" \
